"Contains functions to featurize given structures and train an ExtraTreesRegressor on a given training set."
...
def train_fn(
    ...
    if Path(path_model).exists():
        ...
        logging.info(f"ExtraTreesRegressor model loaded.")
    else:
        if do_training:
            logging.info("Training the model...")
            model = ExtraTreesRegressor(**kwargs_model).fit(df_featurized.to_numpy(), targets)
...
def main():
    ...
    kwargs_model = {
        'max_depth': 10,
        'min_samples_split': 0.0005305243509158821,
        'min_samples_leaf': 0.00046202528982998404,
        'random_state': 42,
    }
    end_model_name = "_An"
