def main():
    ...
    param_grid = {
        ...
        "min_samples_split": list(np.linspace(0.0001, 0.5, 100, endpoint=False)),
    ...
    for i, kwargs_model in enumerate(ParameterGrid(param_grid)):
        if i not in list(range(ifile * 9000, (ifile + 1) * 9000)):
            continue
        ...
        model = ExtraTreesRegressor(**kwargs_model).fit(X_train, y_train)
