"Contains functions to featurize given structures and perform a Gridsearch on the hparams of LGBM on a given training and validation set."
...
def train_fn(
    ...
    if Path(path_model).exists():
        ...
        logging.info("LGBM model loaded.")
    else:
        if do_training:
    ...
    for task in SHG_BENCHMARK_SPLITS:
        ...
        for type_features, name_feat in zip(
            ...
            param_grid = {
                ...
                "min_samples_split": list(np.linspace(0.01, 1, 20, endpoint=False)),
                "min_samples_leaf": list(np.linspace(0.01, 1, 20, endpoint=False)),
