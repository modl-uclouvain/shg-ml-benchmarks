...
def check_need_to_featurize(
    ...
    if Path(path_df).exists():
        ...
        if len(index_feat)==len(ids) and all([i in index_feat for i in ids]):
            print(f"Refeaturizing is not necessary.")
            return False
        print(f"Refeaturizing is necessary.")
    ...
    if type_features:
        ...
        if "mm_fast" in type_features:
            ...
            if need_to_featurize:
                ...
                print("Matminer2024FastFeaturizer features shape:", md.df_featurized.shape)
                md.df_featurized.to_csv(path_df)
                print(f"Matminer2024FastFeaturizer features have been saved at {path_df}")
...
def train_fn(
    ...
    if Path(path_model).exists():
        ...
        print(f"EnsembleMODNet model loaded.")
    else:
        ...
        if path_moddata is not None and Path(path_moddata).exists():
            ...
            raise ValueError(f"If 'path_moddata' does not exist or is None, then either 'df_featurized' or 'moddata' must be provided.")
        print("The MODData has been created.")

        if do_feature_selection:
            print(f"Beginning feature selection...")
            md.cross_nmi = None
            ...
            print(f"Feature selection is now done.")
        ...
        if do_training:
            ...
            print(f"The model has not been trained.")
            model = None
